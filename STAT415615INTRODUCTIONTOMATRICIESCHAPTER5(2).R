...
# a) to define a matrix
# b) to perform basic arithmetic operations on matrices using
#    r code
# c) to find the determinant of a matrix
# d) to find the transpose of a matrix
# e) to find the inverse of a matrix
# f) to produce the identity matrix for a family of matrices provided
# with the dimensions of a matrix.
...
# Let's find the inverse of matrix P.

solve(P)

# and now we confirm that matrix P multiplied by its inverse is the identity
# matrix

solve(P)%*%P

# Now let's consider the matrix Q.

Q <- matrix(c(6, -15, 
             -4,  10), ncol = 2, byrow = TRUE)
Q

# Let's attempt to find an inverse for Q.

solve(Q)  # note that we received an error !!

# Let's now check the determinant. If the determinant is zero,
# the matrix does not have an inverse.

# Checking the determinant.

det(Q)  # the determinant of Q = 0, hence no inverse for matrix Q.

det(P)  # the determinant of P is not 0, hence matrix P does have an 
# inverse as produced earlier.

# As a rule, if a matrix's determinant is 0, the matrix will not have an
# inverse.
